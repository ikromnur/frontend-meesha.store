...
export async function GET(
  ...
  try {
    ...
    const endpoints = [
      ...
      `${BACKEND_URL}/api/discounts/by-code?code=${encodeURIComponent(
        codeOrId
      )}`,
    ...
    if (!found) {
      try {
        ...
        const raw =
          searchRes.data?.data ?? searchRes.data?.items ?? searchRes.data;
    ...
    if (!found) {
      return NextResponse.json(
        {
          success: false,
          message: `Discount with code '${codeOrId}' not found`,
        },
        { status: 404 }
      );
    }
      ...
      startTime:
        normalizeDate(
          (data as Record<string, unknown>)?.startTime ??
            (data as Record<string, unknown>)?.start_date ??
            (data as Record<string, unknown>)?.startDate
        ) ?? (data as Record<string, unknown>)?.startTime,
      endTime:
        normalizeDate(
          (data as Record<string, unknown>)?.endTime ??
            (data as Record<string, unknown>)?.end_date ??
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          axiosError.response?.data?.message || "Failed to fetch discount",
      },
